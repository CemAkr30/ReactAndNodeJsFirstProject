...
import UserFoto from "../assets/Ekran Resmi 2023-07-14 16.47.00.png";
...
export default function Navbar() {
  ...
  const signOut = () => {
    ...
    // Alert mesajını gösterme
    setAlert(true);

    // Zamanlayıcıyı başlatma
    const timeout = setTimeout(() => {
      // Alert mesajını gizleme
      setAlert(false);
    ...
    <Disclosure as="nav" className="bg-white sticky top-0 z-10">
      {({ open }) => (
        <>
          ...
          {alert && (
            <Alert message={"Signing Out"} backColor={"green"} show={alert} />
          )}
        </>
      )}
    </Disclosure>
